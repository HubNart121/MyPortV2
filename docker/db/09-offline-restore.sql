-- Atomically replace all seven backup categories in Local PostgreSQL.
-- Incoming Firebase document IDs are intentionally remapped to PostgreSQL UUIDs.
CREATE OR REPLACE FUNCTION public.restore_backup_v4(payload JSONB)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FOR stock_item IN SELECT value FROM jsonb_array_elements(payload->'stocks') LOOP
    ...
    INSERT INTO public.stocks (
      id, symbol, name, sector, status, asset_type, port_type,
      dividend_per_share, current_price, target_price, graph_url, link_url,
  ...
  FOR item IN SELECT value FROM jsonb_array_elements(payload->'files') LOOP
    INSERT INTO public.files (
      ...
      gen_random_uuid(), item->>'name', NULLIF(item->>'detail', ''),
      NULLIF(item->>'link', ''),
      COALESCE(NULLIF(item->>'created_at', '')::TIMESTAMPTZ, NOW()),
...
REVOKE ALL ON FUNCTION public.restore_backup_v4(JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.restore_backup_v4(JSONB) TO anon;
