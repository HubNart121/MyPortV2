...
export function calcStats(
  ...
  // New annual forecast takes precedence; retain the legacy per-share value for old stocks.
  const expected_dividend_per_share = Number(stock.expected_dividend_per_year ?? 0) || dividend_per_share;
  const expected_dividend = expected_dividend_per_share * active_shares;
  ...
  const expected_profit = (target_price - avg_cost) * active_shares + expected_dividend;
